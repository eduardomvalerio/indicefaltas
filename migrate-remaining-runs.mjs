/**
 * Migration Script Part 2: Complete remaining analise_runs
 * Fetches runs one-by-one to handle large JSONB fields
 */
...
async function main() {
    console.log('🚀 Completando migração de analise_runs...\n');
    ...
    // Get all existing run IDs in MongoDB
    const existingIds = new Set(
        (await runsCollection.find({}, { projection: { _id: 1 } }).toArray()).map(d => d._id)
    );
    console.log(`Runs já no MongoDB: ${existingIds.size}`);

    // Get ALL curvas at once
    const { data: allCurvas } = await cloudClient.from('analise_runs_curvas').select('*');
    console.log(`Curvas totais: ${allCurvas?.length}`);

    // Get all run IDs from cloud (lightweight query)
    const { data: runIds } = await cloudClient
        .from('analise_runs')
        .select('id')
        .order('created_at', { ascending: true });

    const missingIds = (runIds || []).filter(r => !existingIds.has(r.id));
    console.log(`Runs faltando: ${missingIds.length}\n`);
    ...
    for (const { id } of missingIds) {
        try {
            ...
            if (error || !run) {
                console.error(`  ❌ Erro ao buscar run ${id}:`, error?.message);
                errors++;
                continue;
            }

            const runCurvas = (allCurvas || [])
            ...
            imported++;
            if (imported % 10 === 0) console.log(`  ✅ ${imported}/${missingIds.length} runs importados`);
        } catch (err) {
            console.error(`  ❌ Exceção no run ${id}:`, err.message);
            errors++;
        }
    }
    ...
    console.log(`\n🎉 Migração completa!`);
    console.log(`  Importados agora: ${imported}`);
    console.log(`  Erros: ${errors}`);
    console.log(`  Total runs no MongoDB: ${totalRuns}`);
